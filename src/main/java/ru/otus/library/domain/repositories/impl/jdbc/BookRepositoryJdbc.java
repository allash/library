...
public class BookRepositoryJdbc implements BookRepository {
    ...
    public List<DbBook> findAll() {
        ...
        Map<Long, BookResultSet> bookResultSetMap = new HashMap<>();
    ...
    private void createOrUpdate(KeyHolder keyHolder, DbBook book) {
        namedParamsJdbcOperations.getJdbcOperations().update(con -> {
            ...
            if (book.getId() != null) { preparedStatement.setLong(2, book.getId()); }
            return preparedStatement;
        }, keyHolder);
    }

    private void cleanBookGenresByBookId(Long bookId) {
        namedParamsJdbcOperations.getJdbcOperations().update("DELETE FROM book_genre WHERE book_id = ?", bookId);
    }

    private void cleanBookAuthorsByBookId(Long bookId) {
        namedParamsJdbcOperations.getJdbcOperations().update("DELETE FROM book_author WHERE book_id = ?", bookId);
    }

    private void batchUpdateGenres(Long bookId, final List<DbGenre> genres) {
    ...
    private void batchUpdateAuthors(Long bookId, final List<DbAuthor> authors) {
